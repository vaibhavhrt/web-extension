...
const enterprise_platformKeysCompletionItemProvider = {
    provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
        let linePrefix = document.lineAt(position).text.substr(0, position.character);
        if (!linePrefix.endsWith('chrome.enterprise.platformKeys.')) {
            return undefined;
        }
...
export default enterprise_platformKeysCompletionItemProvider;
