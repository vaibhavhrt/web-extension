...
const certificateProviderCompletionItemProvider = {
    provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
        let linePrefix = document.lineAt(position).text.substr(0, position.character);
        if (!linePrefix.endsWith('chrome.certificateProvider.')) {
            return undefined;
        }
...
export default certificateProviderCompletionItemProvider;
